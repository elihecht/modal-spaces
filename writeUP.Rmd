...
knitr::opts_knit$set(root.dir = "/Users/eli/Documents/modal-spaces-master")
# knitr::opts_knit$set(root.dir = "C:/Users/jonat/Dropbox (Dartmouth College)/modalBackgrounds/modal-spaces-master")
```
...
\> Consider one of the things you believed [Agent] could do: [Participant response]. \> *Probability Question:* How likely is it that [Agent] will do that thing?\
...
## This allows us to write out the data at this point - was used for NLP paper with Gokul

# pgRatings <- pgW %>% select(id, S1A1_4:S18A5_6, -c(demo:ses, contains("FL"), contains("."))) %>% 
...
# 
# 
# pgText <- pgW %>% select(id, contains(".FR"), -c(demo:ses, contains("FL"))) %>% 
#   pivot_longer(-id, names_to = "question", values_to = "response",
#                values_drop_na = T) %>% 
#   separate(question, into = c("context", "answer"), sep = "_") %>% 
#   mutate(across(2:3, parse_number)) %>% 
#   mutate(answer = answer-3)
# 
# 
# pgGokul <- pgRatings %>% pivot_wider(names_from=c(question),values_from=c(response)) %>%
#             left_join(pgText, by=c("id","context","answer")) %>%
#             write.csv(file="data/EliLongGenData.csv",row.names = F)
...
if(file.exists("lmerOutputs/pgNormInt.rda")){
  ...
  pgNormInt$p <- pgNormInt$`Pr(>Chisq)`
  pgNormP$p <- pgNormP$`Pr(>Chisq)`
  pgNormM$p <- pgNormM$`Pr(>Chisq)`
  
}

# Here we predict the order in which an answer was generated based on how good the answer is
if(file.exists("lmerOutputs/pgPredictP.rda")){
  ...
  # pgPredictP$p <- pgPredictP$"Pr(>Chisq)"
  # pgPredictM$p <- pgPredictM$"Pr(>Chisq)"
  # pgPredictN$p <- pgPredictN$"Pr(>Chisq)"
}
...
The better an option is, the earlier it tended to be generated by a participant ($F$(`r pgPredict[[1]][["Df"]][1]`$) =$ `r round(pgPredict[[1]][["F value"]][1],2)`, $p <$ `r max(.001, round(pgPredict[[1]][["Pr(>F)"]][1],3))`). Further, independent of normality and morality, the more probable a response was, the more likely it was to be generated earlier ($\chi^2$(`r pgPredictP$Df[[2]]`) = `r pgPredictP$Chisq[[2]] %>% round(2) %>% format(scientific=F)`, $p <$ `r max(.001, round(pgPredictP$"Pr(>Chisq)"[[2]], 3))`). The same is true for morality, where independent of the other ratings, the more moral a possibility was, the more likely it was to be generated earlier ($\chi^2$(`r pgPredictM$Df[[2]]`) = `r round(pgPredictM$Chisq[[2]], 2) %>% format(scientific=F)`, $p <$ `r max(.001, round(pgPredictM$"Pr(>Chisq)"[[2]], 3))`). Normality was not found to have a similar independent predictive effect, ($\chi^2$(`r pgPredictN$Df[[2]]`) = `r round(pgPredictN$Chisq[[2]], 2)`, $p =$ `r max(.001, round(pgPredictN$"Pr(>Chisq)"[[2]], 3))`). This is likely because, as will be seen above, normality is jointly predicted by probability and morality, much of the variance in answer generation number is already accounted for by these ratings.

<!-- What's the best way to do citations in this format?

Answer: I think you can actually use latex citations, we just need to make sure we have the .bib file specified. 
--> 

We sought to replicate the finding of Bear and Knobe (2017) that normality judgments are predicted by judgments of morality and probability. First we constructed a linear mixed-effects model to predict normality ratings with probability and morality ratings as predictors and fixed effects for context and [not positive on how to refer to the structure of the model]. The model performed significantly worse when morality and probability were removed. ($\chi^2$(`r pgNormInt$Df[[2]]`) = `r round(pgNormInt$Chisq[[2]], 2)`, $p <$ `r max(.001, round(pgNormInt$"Pr(>Chisq)"[[2]], 3))`). We further found that both ratings were independently predictive of normality. The model performed worse when probability was removed ($\chi^2$(`r pgNormP$Df[[2]]`) = `r round(pgNormP$Chisq[[2]], 2)`, $p <$ `r max(.001, round(pgNormP$"Pr(>Chisq)"[[2]], 3))`), and when morality was removed ($\chi^2$(`r pgNormM$Df[[2]]`) = `r round(pgNormM$Chisq[[2]], 2)`, $p <$ `r max(.001, round(pgNormM$"Pr(>Chisq)"[[2]], 3))`). We will here on out use normality as a measure of the value of an action, unless we state otherwise.
...
# includes participant answers as well as what they are grouped into
coded <- codingKey %>%
...
# for each context do an independent samples t test comparing ratings of answers in the 3 most commonly used action categories to all other answers
...
This pattern held true within each context. For each context, we performed an independent samples t-test comparing average ratings for answers in the three most popular action categories to answers outside of these categories. All t-values were in the expected direction, with $p<.001$ for all but `r t_table %>% summarise(sum(p > .001)) %>% pull()` of the 18 contexts.
...
evL <- evW %>% select(id, S1A:S18A) %>% 
  ...
  pivot_wider(names_from = question, values_from = response ) %>% ## this creates the average for all three judgments
                mutate(avg=(Normality+Probability+Morality)/3) %>%
                pivot_longer(cols=c(Normality,Morality,Probability,avg),
                             names_to = "question",values_to ="response")
...
for (c in unique(pgL$context)){
  ...
  # for each action, calculates the proportion of samples that are rated higher on average, higher by morality and higher by probability than that action
  for (a in 1:6){
    modalDistTable <- evSum %>%
      ...
      slice(a:a) %>%
      mutate(modalDistance = length(pgSamp$avg[mean<pgSamp$avg])/length(pgSamp$avg),
  ...
  #     filter(context == c, question == 'avg') %>% 
  #     slice(a:a) %>%
  #     mutate(modalDistance = length(pgSamp[mean<pgSamp])/length(pgSamp)) %>%
  #     bind_rows(modalDistTable)
  # }
}
#print(c13.h)
...
# htL %>% summarise(mean = mean(response)) %>% pull() %>% round(1)
...
<!-- [EH: I'm not sure the best way to explain the Icard model we're comparing our against. Jonathan do you mind adding a bit of context here?] -->
...
\>*Counterfactual relevance statement:* Given the situation they were in, how relevant is it to consider the possibility that [Agent] could have done something other than deciding to [Chosen action].\ 
...
# Read in the causal judgments for each action
...
#icardCorr <- with(causalSum, cor.test(icard_model,causal))
#icardModCorr <- with(causalSum, cor.test(icard_modified,causal))
# CFsCorr <- with(causalSum, cor.test(suffCF, counterfactual_necessity))
# suffCorr <- with(causalSum, cor.test(suffCF, causal))

# with(causalSum, cor.test(causal, modalDistance))
# with(causalSum, cor.test(causal, counterfactual_necessity))

# lmers + anova to demonstrate separate effects
...
# causalL %>% summarise(mean(response)) %>% pull() %>% round(1) %>% format(nsmall = 1)
# counterL %>% summarise(mean(response)) %>% pull() %>% round(1) %>% format(nsmall = 1)
# suffL %>% summarise(mean(response)) %>% pull() %>% round(1) %>% format(nsmall = 1)
...
suffAnovas <- list(
  ...
  anova(lmer3, lmer4))	
# c(icardCorr, CFsCorr, suffCorr)	

modalRelevanceCorr <- with(causalSum,cor(modalDistance,counterfactual_relevance))
...
# ggsave(fig5_top,file="figs/fig5_top.jpg",width = 12,height = 10)
# ggsave(fig5_bottom,file="figs/fig5_bottom.jpg",width = 12,height = 4)
...
potency3 <-  causalSum %>% 
  ...
  ggplot(aes(x = reorder(action, modalDistance), 
             y = counterfactual_c, 
             label = action, fill = modalDistance)) +
  labs(x = "Actual Action", y = "Counterfactual Potency", fill = "Modal Distance") +
  geom_col() +
  facet_wrap(~context,scales = "free_x") +
  theme_bw() +
  theme(
...
```{r fig6, echo=F,warning=F,message=F}
fig6_top <- blameSum %>% mutate(context=factor(context)) %>%
                  ggplot(aes(y=blame,x=necessity_strength)) +
                  geom_point(aes(color=context)) +
                  labs(y="Agreement that the agent should be blamed", 
                       x="Counterfactual potency given the actual action") +
                  geom_smooth(method=lm)+
                  theme_bw() +
                  theme(
                    plot.background = element_blank()
...
g <- ggplot_build(fig6_top)
colors <- unique(g$data[[1]]$colour)
colors <- colors[c(1,13,15)]


fig6_bottom <-  blameSum %>% mutate(context=factor(context)) %>%
  filter(context %in% c("1","13","15")) %>%
...
ggsave(fig6_top,file="figs/fig6_top.jpg",width = 12,height = 10)
ggsave(fig6_bottom,file="figs/fig6_bottom.jpg",width = 12,height = 4)
...
# 
# # predicting normality ratings from probability and morality
# if(file.exists("lmerOutputs/altpgNorm0.csv")){
#   # saves computational time to just read these tables instead of recalculating them every time
#   pgNorm0 <- read.csv("lmerOutputs/altpgNorm0.csv")
#   pgNormP <- read.csv("lmerOutputs/altpgNormP.csv")
#   pgNormM <- read.csv("lmerOutputs/altpgNormM.csv")
# } else{
#   lmerNorm_Full <- lmer(Normality ~ Morality + Probability + (1|context) +
#                    (Probability + Morality |id), data=altpgPredict)
#   lmerNorm_0 <- lmer(Normality ~  (1|context) +
#                    (Probability + Morality |id), data=altpgPredict)
#   lmerNorm_P <- lmer(Normality ~ Morality + (1|context) +
#                    (Probability + Morality |id), data=altpgPredict)
#   lmerNorm_M <- lmer(Normality ~ Probability + (1|context) +
#                    (Probability + Morality |id), data=altpgPredict)
#   pgNorm0 <- anova(lmerNorm_Full,lmerNorm_0)
#   pgNormP <- anova(lmerNorm_Full,lmerNorm_P)
#   pgNormM <- anova(lmerNorm_Full,lmerNorm_M)
#   
#   pgNorm0$p <- pgNorm0$"Pr(>Chisq)"
#   pgNormP$p <- pgNormP$"Pr(>Chisq)"
#   pgNormM$p <- pgNormM$"Pr(>Chisq)"
# }
# 
...
Once again, the better an option is, the earlier it tended to be generated by a participant ($F$(`r altpgPredict[[1]][["Df"]][1]`$) =$ `r round(altpgPredict[[1]][["F value"]][1],2)`, $p <$ `r max(.001, round(altpgPredict[[1]][["Pr(>F)"]][1],3))`). Further, independent of normality and morality, the more probable a response was, the more likely it was to be generated earlier ($\chi^2$(`r pgPredictP$Df[[2]]`) = `r round(pgPredictP$Chisq[[2]], 2)`, $p <$ `r max(.001, round(pgPredictP$"Pr(>Chisq)"[[2]], 3))`). The same is true for morality, where independent of the other ratings, the more moral a possibility was, the more likely it was to be generated earlier ($\chi^2$(`r pgPredictM$Df[[2]]`) = `r round(pgPredictM$Chisq[[2]], 2)`, $p <$ `r max(.001, round(pgPredictM$"Pr(>Chisq)"[[2]], 3))`). Once again, normality was not found to have a similar independent predictive effect, ($\chi^2$(`r pgPredictN$Df[[2]]`) = `r round(pgPredictN$Chisq[[2]], 2)`, $p =$ `r max(.001, round(pgPredictN$"Pr(>Chisq)"[[2]], 3))`).\

We then constructed a model identical to the one used earlier to predict normality ratings for possibilities generated for the new contexts. using morality and probability ratings. Once again, the model performed significantly worse when morality and probability were removed. ($\chi^2$(`r pgNormInt$Df[[2]]`) = `r round(pgNormInt$Chisq[[2]], 2)`, $p <$ `r max(.001, round(pgNormInt$"Pr(>Chisq)"[[2]], 3))`). We further found that both ratings were independently predictive of normality. The model performed worse when probability was removed ($\chi^2$(`r pgNormP$Df[[2]]`) = `r round(pgNormP$Chisq[[2]], 2)`, $p <$ `r max(.001, round(pgNormP$"Pr(>Chisq)"[[2]], 3))`), and when morality was removed ($\chi^2$(`r pgNormM$Df[[2]]`) = `r round(pgNormM$Chisq[[2]], 2)`, $p <$ `r max(.001, round(pgNormM$"Pr(>Chisq)"[[2]], 3))`).
...
# 
# ggsave(taxi_shift, file="figs/taxiDistShift.png",width=4.5, height=6, units="in")
...
# ggsave(figN_top,file="figs/figN_top.jpg",width = 12,height = 10)
...
altCompare <- althtSum %>% 
  ...
  mutate(modalDiff = modalDistance - altmodalDistance,
...
lmer0 <- lmer(data=altCompare %>% filter(context != 11), hadTo ~ altmodalDistance + modalDistance + (1|context))
# original modal distance removed
lmer1 <- lmer(data=altCompare%>% filter(context != 11), hadTo ~ altmodalDistance + (1|context))
# shifted modal distance removed
lmer2 <- lmer(data=altCompare%>% filter(context != 11), hadTo ~ modalDistance + (1|context))

# full model predicting shifted context force judgments
lmer3 <- lmer(data=altCompare %>% filter(context != 11), althadTo ~ altmodalDistance + modalDistance + (1|context))
# original modal distance removed
lmer4 <- lmer(data=altCompare%>% filter(context != 11), althadTo ~ modalDistance + (1|context))
# shifted contextr model distance removed
lmer5 <- lmer(data=altCompare%>% filter(context != 11), althadTo ~ altmodalDistance + (1|context))


htModelComps <- list(anova(lmer0, lmer1), anova(lmer0, lmer2),
                     anova(lmer3, lmer4), anova(lmer3, lmer5))
...
We also constructed two models for predicting force judgments. We predicted force judgments for the original contexts using both the original modal distance value and the new modal distance value for each action. The model performed significantly worse when the original modal distance values were removed ($\chi^2$(`r htModelComps[[1]]$Df[[2]]`) = `r round(htModelComps[[1]]$Chisq[[2]], 2)`, $p <$ `r max(.001, round(htModelComps[[1]]$"Pr(>Chisq)"[[2]], 3))`), but was not effected by the removal of the altered context modal distance values ($\chi^2$(`r htModelComps[[2]]$Df[[2]]`) = `r round(htModelComps[[2]]$Chisq[[2]], 2)`, $p =$ `r max(.001, round(htModelComps[[2]]$"Pr(>Chisq)"[[2]], 3))`).\
The second model predicted force judgments for the altered contexts using the same terms. As expected, the model performed worse without the modal distance values from the altered contexts ($\chi^2$(`r htModelComps[[3]]$Df[[2]]`) = `r round(htModelComps[[3]]$Chisq[[2]], 2)`, $p <$ `r max(.001, round(htModelComps[[3]]$"Pr(>Chisq)"[[2]], 3))`). Interestingly, the model also performed worse without the original context modal distance values ($\chi^2$(`r htModelComps[[4]]$Df[[2]]`) = `r round(htModelComps[[4]]$Chisq[[2]], 2)`, $p =$ `r max(.001, round(htModelComps[[4]]$"Pr(>Chisq)"[[2]], 3))`).\
...
acrossCorrData <- acrossCorrData %>% mutate(modalSum = -1) 
for (c in unique(acrossCorrData$context)){
  ...
  for (a in 1:6){
    ...
    a_val <- acrossCorrData %>% filter(actionNumb == a_numb) %>% pull(mean)
    
    acrossCorrData <- acrossCorrData %>% mutate(modalSum = if_else(actionNumb == a_numb, sum(pgSamp[a_val < pgSamp]), modalSum))
...
# acrossCorrData %>% with(list(cor.test(modalSum, hadTo), cor.test(modalDistance, hadTo)))
...
for (subj in unique(withinCorrData$id)){  ## for each participant
    ...
    for (c in unique(d$context)){ 
        ...
        ## for each actual action, calculate the proportion of the participant's own samples that were rated as more normal than the avg rating
        action_numb <- d$actionNumb[d$context==c]
        a_val <- evSum$mean[evSum$actionNumb==action_numb & evSum$question=="avg"]
        withinCorrData$sampleSum[withinCorrData$id==subj & withinCorrData$context==c] <- sum(pgSamp[a_val<pgSamp])
        withinCorrData$sampleDistance[withinCorrData$id==subj & withinCorrData$context==c] <- length(pgSamp[a_val<pgSamp])/length(pgSamp[])
...
# cor.test(withinCorrData$modalDistance,withinCorrData$sampleDistance)
# cor.test(withinCorrData$hadTo,withinCorrData$sampleDistance)
# cor.test(withinCorrData$hadTo,withinCorrData$modalSum)
...
withinCorr <- list(anova(lmer1, lmer3),anova(lmer1, lmer2), # comparing modal and sample distance measures 
...
First, we calculated a 'participant distance' measure analogous to the modal distance measure. For the participant distance measure, for each action a participant viewed, we calculated the proportion of alternative actions they came up with for that context that they rated higher than the average value for the action. To investigate the extent to which participant distance explained variance unaccounted for by modal distance, we constructed a linear mixed effects model with modal distance and participant distance as fixed effects and a random effect for context. The model performed worse when modal distance was removed ($\chi^2$(`r withinCorr[[1]]$Df[[2]]`) $=$ `r round(withinCorr[[1]]$Chisq[[2]], 2)`, $p <$ `r max(.001,round(withinCorr[[1]]$"Pr(>Chisq)"[[2]],3))`) and when participant distance was removed ($\chi^2$(`r withinCorr[[2]]$Df[[2]]`) $=$ `r round(withinCorr[[1]]$Chisq[[2]], 2)`, $p =$ `r max(.001,round(withinCorr[[2]]$"Pr(>Chisq)"[[2]],3))`). \
...
Once again, we constructed a linear mixed effects model using the sample sum and modal sum as fixed effects and a random effect for context. The model performed significantly worse when modal sum was removed ($\chi^2$(`r withinCorr[[3]]$Df[[2]]`) $=$ `r round(withinCorr[[3]]$Chisq[[2]], 2)`, $p <$ `r max(.001,round(withinCorr[[3]]$"Pr(>Chisq)"[[2]],3))`), and when sample sum was removed ($\chi^2$(`r withinCorr[[4]]$Df[[2]]`) $=$ `r round(withinCorr[[4]]$Chisq[[2]], 2)`, $p =$ `r max(.001,round(withinCorr[[4]]$"Pr(>Chisq)"[[2]],3))`). However, note that sample sum accounted for a much smaller amount of the variance than modal sum. This suggests that while sampling specific alternative actions may play some role in high-level judgments, most of the work is likely being done by a heuristic representation.
