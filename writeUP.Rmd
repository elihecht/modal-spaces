...
# percentage of time that a participant rates each answer for a given scenario highest. ties go to the first answer
perBestAlt <- altpgL %>% filter(question=="avg") %>%
  group_by(id, context) %>% 
  mutate(rank = rank(-response, ties.method = "first")) %>% 
  group_by(answer) %>% 
  summarise(numBest = sum(rank == 1), .groups = "drop") %>% 
...
#  anova predicting answer number by average rating of an answer
altpgPredictAnova <- summary(aov(lm(as.numeric(answer) ~ avg, data=altpgPredict)))
...
actualAction2 <- "stealing someone else’s ring"
    ...
    ggplot(aes(x = abnomrality)) +
      ...
      geom_label(data = evSum %>% filter(context == contextChoice & action==actualAction) %>%
                                     ...
                                     mutate(abnormality=100-mean,
                                            ...
                                            action=factor(c("call taxi")[action]) 
                                            ), 
  ...
  mutate(abnomrality=100-response) %>%
    ggplot(aes(x = abnomrality)) +
      ...
      geom_label(data = evSum %>% filter(context == contextChoice & action==actualAction) %>%
                                     ...
                                     mutate(abnormality=100-mean,
                                            action=factor(action),
                                            action=factor(c("steal ring")[action])
                                            ), 
                 aes(x = abnormality, y = -0.0005, 
                     label = action), 
                 color="blue",
...
# ggsave(fig6bottomR, file="figs/fig6bottomR.jpg",width=4.5, height=6, units="in")


# the following figures are not included in the main text or appendix

## replication figure
figReplication <- althtSum %>% mutate(context=factor(context)) %>%
...
figCompare <- altCompare %>% mutate(context=factor(context)) %>%
...
## Calculating sample distance for each participant as the proportion of actions where v(a_a) < v(a_s) and sample sum for each participant as the sum of values where v(a_a) < v(a_s)
...
## comparing the original proportional modal distance measure and the more continuous participant-specific measure. 


# full model, including both sampleSum and modalDistance
lmer0 <- lmer(data=withinCorrData, scale(hadTo) ~ scale(sampleSum) + scale(modalDistance) +
         (scale(sampleSum) + scale(modalDistance) |context) +
         (1|id))

# removing modalDistance
lmer1 <- lmer(data=withinCorrData, scale(hadTo) ~ scale(sampleSum) + 
                (scale(sampleSum) + scale(modalDistance)|context) +
                (1|id))
...
modalDist_vs_sampleSum <- anova(lmer0,lmer1) 
sampleSum_vs_modalDist <- anova(lmer0,lmer2)
...
Finally, for completeness, we compared the original proportional modal distance measure with the continuous modal sum metric. Model comparisons once again revealed a significant effect of modal distance ($\chi^2$(`r modalDist_vs_sampleSum$Df[[2]]`) $=$ `r round(modalDist_vs_sampleSum$Chisq[[2]], 3)`, $p <$ `r max(.001,round(modalDist_vs_sampleSum$"Pr(>Chisq)"[[2]],3))`) and a smaller but still significant effect of sample sum ($\chi^2$(`r sampleSum_vs_modalDist$Df[[2]]`) $=$ `r round(sampleSum_vs_modalDist$Chisq[[2]], 3)`, $p <$ `r max(.001,round(sampleSum_vs_modalDist$"Pr(>Chisq)"[[2]],3))`).
