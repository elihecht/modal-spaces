...
# Chosen optimal number of clusters for KMeans for each context
# These values were selected based on the results of the elbow plots for each context
kMeans_klist <- c(4, 5, 8, 3, 5, 3, 5, 6, 4, 4, 3, 7, 7, 7, 3, 7, 6, 6)
...
# Add k-means and DBSCAN clustering results to clustering_outputs
clustering_outputs = data.frame()
for (context in 1:18){
  # read in kmeans results, reading from appropriate folder depending on the context
  kmeans_output <- read.csv(paste0('../nlpCoding/pg_clusters/KMeans/k',kMeans_klist[context], "/tables/S", context, ".csv"), row.names = "X") 
  kmeans_output <- kmeans_output %>% 
    rename(cluster_K = cluster, cluster_name_K = cluster_name) %>% 
    mutate(cluster_K = cluster_K + 1)
  
  # read in DBSCAN results
  DBSCAN_output <- read.csv(paste0('../nlpCoding/pg_clusters/DBSCAN/eps',eps, '_samp', minSamp,'/tables/S', context, '.csv'), row.names = 'X')
  DBSCAN_output <- DBSCAN_output %>% 
    rename(cluster_DB = cluster, cluster_name_DB = cluster_name) %>% 
    select(cluster_DB, cluster_name_DB)
  ...
  # add DBSCAN clusters to text embedding and kmeans clusters data
  merged_outputs = bind_cols(kmeans_output, DBSCAN_output, .name_repair = "unique")
  
  # add outputs from this scenario to clustering_outputs dataframe
  clustering_outputs = bind_rows(clustering_outputs, merged_outputs)
  
}

# cluster_df contains the possibility generation responses and their cluster assignments for both clustering methods
cluster_df <- clustering_outputs %>% select(source, id, context, answer, response_original, value, cluster_K, cluster_name_K, cluster_DB, cluster_name_DB) %>% 
  mutate(cluster_K = as.integer(cluster_K),
         cluster_DB = as.integer(cluster_DB))
```
In addition to manually coding participant responses, we also used natural language processing to group similar responses. We computed sentence embeddings for all responses in each scenario using [Sentence-BERT](www.sbert.net) and reduced the dimensionality of the embedding space using [UMAP](umap-learn.readthedocs.io/en/). We clustered the resulting embeddings using two methods, DBSCAN and k-means. For k-means, we performed clustering at a range of k-values and determined the ideal number of clusters for each context by visual inspecting the resulting [elbow plots](../nlpCoding/pg_clusters/KMeans/kplots/). For DBSCAN we selected values for $\epsilon$, or neighborhood size, and minimum number of samples by testing a range of values and visually inspecting the resulting the plots. The analysis used is provided in [nlpCoding.ipynb](../nlpCoding/nlpCoding.ipynb).

Using the elbow method to determine optimal number of clusters for k-means, the number of clusters by context ranged from `r cluster_df %>% group_by(context) %>% summarize(max(cluster_K)) %>% pull() %>% min()` to `r cluster_df %>% group_by(context) %>% summarize(max(cluster_K)) %>% pull() %>% max()` clusters. For DBSCAN, we selected values of $\epsilon =$ `r eps` and $minSamples =$ `r minSamp` as the parameters that resulted in the most semantically-meaningful clusters across contexts. At these values, the number of clusters per context ranged from `r cluster_df %>% group_by(context) %>% summarize(max(cluster_DB)) %>% pull() %>% min()` to `r cluster_df %>% group_by(context) %>% summarize(max(cluster_DB)) %>% pull() %>% max()`.


#### Results
```{r NLPcodingresults, warnings = F, echo=F,message=F}
# rank k-means clusters for each context by number of answers 
clustersRankedK <- cluster_df %>%
  group_by(context, cluster_K, cluster_name_K) %>% 
  summarise(n = n(), mean = mean(value), sd = sd(value)) %>% 
  group_by(context) %>% 
  mutate(rank = rank(-n, ties.method = "f")) %>% 
  unite("context_cluster", context, cluster_K)


# adds columns with information about whether a given answer is in the top 1 or 3 most picked k-means clusters for that context
cluster_df <- cluster_df %>% unite("context_cluster", context, cluster_K, remove = F) %>%
  ungroup() %>% 
  mutate(best_K = context_cluster %in%
           (clustersRankedK %>% filter(rank == 1) %>% pull(context_cluster)), 
         top3_K = context_cluster %in%
           (clustersRankedK %>% filter(rank <= 3) %>% pull(context_cluster))) %>%
  separate(context_cluster, into = c("context", "cluster_K"), sep = "_")

## repeat above but with DBSCAN clusters
...
# percentage of participants who didn't come up with any of the most picked answers for that context. Calculated for both k-means and DBSCAN clusters
perBestClusterNLP <- cluster_df %>% 
  group_by(context, id) %>% 
  summarise(noneBest_K = !any(best_K),
            noneTop3_K = !any(top3_K),
            noneBest_DB = !any(best_DB),
            noneTop3_DB = !any(top3_DB)) %>%
  ungroup() %>% 
  summarise(noneBest_K = sum(noneBest_K)/n(),
            noneTop3_K = sum(noneTop3_K)/n(),
            noneBest_DB = sum(noneBest_DB)/n(),
...
ttable_NLP <- cluster_df %>%
  ...
  summarise(
    ...
    #           if_else(
    #             more_than_3, 
    #             # "list(t.test(value ~ top3_K))",
    #                  NA),
            t_test_result_DB = list(t.test(value ~ top3_DB))) %>%
  ungroup() %>% 
  # spread out results for two t tests
  mutate(
    # t_K = sapply(t_test_result_K, function(x) x$statistic),
    # p_K = sapply(t_test_result_K, function(x) x$p.value),
    # df_K = sapply(t_test_result_K, function(x) x$parameter),
...
Using the clustering results with the DBSCAN alogirthm, as with manually grouped responses, we found striking convergence across participant's answers within each context. Only `r round(summarise(cluster_df, otherPer = sum(cluster_DB==0)/n())[[1]] * 100,1)`% of answers were labeled as 'outliers' by the algorithm, analogous to our finding for 'other' responses using manual coding. Across contexts, only `r round(perBestClusterNLP[[1,4]] * 100, 1)`% of participants did not come up with any of the 3 most common answers in their set of 5 possible actions, and only `r round(perBestClusterNLP[[1,3]] * 100, 1)`% of participants didn't put down the most popular answer for that context.
...
We found similar results using the clustering results from the k-means algorithm. There is no equivalent 'outlier' or 'other' labeling in k-means, so percentage of responses being classified as 'other' was not calculated. Across contexts, only `r round(perBestClusterNLP[[1,2]] * 100, 1)`% of participants did not come up with any of the 3 most common answers in their set of 5 possible actions, and only `r round(perBestClusterNLP[[1,1]] * 100, 1)`% of participants didn't put down the most popular answer for that context. 

Because of the low number of clusters (`r cluster_df %>% group_by(context) %>% summarize(max(cluster_K)) %>% pull() %>% min()`-`r cluster_df %>% group_by(context) %>% summarize(max(cluster_K)) %>% pull() %>% max()`) obtained using k-means with the elbow-method, we did not compare actions in the three most common categories to actions in the other categories.
...
# calculate average value of and proportion of possibility generation responses for each k-means cluster
pg_groups <- coded %>% 
...
# Chosen optimal number of clusters for KMeans for each context
# these numbers were selected based on the results of the elbow plots for each context
kMeans_klist <- c(4, 8, 8, 5, 7, 6, 5, 6, 4, 4, 3, 5, 6, 6, 4, 8, 7, 6)
...
clustering_outputs = data.frame()
for (context in 1:18){
  # read in kmeans results
  kmeans_output <- read.csv(paste0('../nlpCoding/pg_decision_clusters/KMeans/k',kMeans_klist[context], "/tables/S", context, ".csv"), row.names = "X") 
  kmeans_output <- kmeans_output %>% 
    rename(cluster_K = cluster, cluster_name_K = cluster_name) %>% 
    mutate(cluster_K = cluster_K + 1)
  
  # read in DBSCAN results
  DBSCAN_output <- read.csv(paste0('../nlpCoding/pg_decision_clusters/DBSCAN/eps',eps, '_samp', minSamp,'/tables/S', context, '.csv'), row.names = 'X')
  DBSCAN_output <- DBSCAN_output %>% 
    rename(cluster_DB = cluster, cluster_name_DB = cluster_name) %>% 
    select(cluster_DB, cluster_name_DB)
...
decision_clusters <- clustering_outputs %>% select(source, id, context, answer, response_original, value, cluster_K, cluster_name_K, cluster_DB, cluster_name_DB)


```

Once again, in addition to manually coding participant responses, we also used natural language processing to group similar responses. We computed sentence embeddings for all responses in each scenario using [Sentence-BERT](www.sbert.net) and reduced the dimensionality of the embedding space using [UMAP](umap-learn.readthedocs.io/en/). We clustered the resulting embeddings using two methods, DBSCAN and k-means. For k-means, we performed clustering at a range of k-values and determined the ideal number of clusters for each context by visual inspecting the resulting [elbow plots](../nlpCoding/pg_clusters/KMeans/kplots/).

Using the elbow method to determine optimal number of clusters for k-means, the number of clusters by context ranged from `r decision_clusters %>% group_by(context) %>% summarize(max(cluster_K)) %>% pull() %>% min()` to `r decision_clusters %>% group_by(context) %>% summarize(max(cluster_K)) %>% pull() %>% max()` clusters. For DBSCAN, we once again used values of $\epsilon =$ `r eps` and $minSamples =$ `r minSamp` as the parameters that resulted in the most semantically-meaningful clusters across contexts. At these values, the number of clusters per context ranged from `r decision_clusters %>% group_by(context) %>% summarize(max(cluster_DB)) %>% pull() %>% min()` to `r decision_clusters %>% group_by(context) %>% summarize(max(cluster_DB)) %>% pull() %>% max()`.
...
# calculate average value of and proportion of possibility generation responses for each k-means cluster
pg_clusters_K <- decision_clusters %>%  
  filter(source == "pg") %>% 
  group_by(context, cluster_K, cluster_name_K) %>% 
...
# calculate proportion of decision responses for each context being grouped into each cluster
dec_clusters_K <- decision_clusters %>% 
...
# run decision model for k-means clusters at all consideration set sizes
decision_results_K <- map_df(k_values,
                             ~ run_decision_model(.x,
                                                  sample_sets,
                                                  decision_clusters, 
                                                  pg_clusters_K, 
...
# table with k-means clusters decision model correlations and p values for 1 <= k <= 10
k_correlations_K <- decision_results_K %>% 
  group_by(k) %>%
  summarise(correlation = cor.test(model_proportion, proportion_decision)$estimate,
            p_value = cor.test(model_proportion, proportion_decision)$p.value) 
...
# selects results from decision model with consideration set size k=3 in line with pre-existing models
cor_result_K <- with(decision_results_K %>% filter(k==3), cor.test(model_proportion, proportion_decision))
cor_result_DB <- with(decision_results_DB %>% filter(k==3), cor.test(model_proportion, proportion_decision))


```
We used the same decision making model as previously, this time predicting the likelihood of a decision in a given NLP-cluster rather than a manually coded action category being selected for the first-person decision study.

Using both the DBSCAN and the k-means clusters, the model once again strongly predicted actual decision likelihood at all consideration set sizes tested (`r round(min(k_correlations_DB$correlation), 3)` $<r<$ `r round(max(k_correlations_DB$correlation),3)` using DBSCAN clusters, and `r round(min(k_correlations_K$correlation), 3)` $<r<$ `r round(max(k_correlations_K$correlation),3)` using k-means clusters, all at $p<.001$).
