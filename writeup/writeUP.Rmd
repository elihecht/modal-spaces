...
# ggsave(fig1,file="figs/fig1.jpg",width = 10,height = 7)
...
# ggsave(figS1,file="figs/figS1.jpg",width = 7,height = 7)
...
# ggsave(codedHist13,file="figs/fig2.jpg",width = 12,height = 10)
```
...
of contextually relevant possibilities, but in demonstrating that these repre-
sentations play a domain-general role in high-level cognition. We first tested
...
Instead of manually coding the first-person decision responses, we used natural language processing to group similar responses. First, we aggregated the responses from the original possibility generation study and the decision study, stripping all agent names and pronouns from the responses. We then computed sentence embeddings for all responses using Sentence-BERT ([www.sbert.net](www.sbert.net)), reduced the dimensionality of the embedding space using UMAP ([umap-learn.readthedocs.io/en/](umap-learn.readthedocs.io/en/)) , and performed k-means clustering on the resulting embeddings, with the number of clusters selected using the elbow method. The analysis used is provided in "nlpCoding/decision_NLP.ipynb". 

```{r clusterRead, echo=F,warning=F,message=F}

cluster_tables_numManual <- list.files('../nlpCoding/numSameAsManual/cluster_tables', full.names = T)
cluster_tables_numElbow <- list.files('../nlpCoding/numFromElbow/cluster_tables', full.names = T)
...
# # Define a function for text splitting along colon
# split_text <- function(x) {
#   s <- unlist(strsplit(x, ":", fixed = TRUE))
#   return(trimws(s[length(s)]))
# }


cluster_df <- clustering_outputs %>% 
  # mutate(is_centroid = cluster_name == sapply(merged_text, split_text)) %>% 
  select(id, context, answer, response_original, source, value, clustering_method, cluster, cluster_name, 
         # is_centroid,
         )

# filters to just responses from the possibility generation study, with clusters determined using the same number as the manual groupings
...
# joins the above dataframe with the manual groupings
...
# Qualitatively, viewing this dataframe suggests allignment in the manual and NLP groupings
...
kappaAgreement <- kappa2(cluster_compare[, c("biggest_manual_group_by_cluster", "manual_group")])


```

Because we used K-means clustering, we had to specify the number of clusters to be used for each scenario. First we set the number of clusters for each context to be identical to the number of manual groups used for that context, in order to directly compare the performance of our manual clustering and the data-driven method.

Qualitatively, the clusters generated made were semantically meaningful, with similar types of answers being grouped together, even when the exact words differed (i.e. "" and "" were placed in the same cluster). We further calculated the agreement between the clustering method and our manual groupings. Each cluster was assigned to a manual grouping by determining which manual grouping accounted for a plurality of the cluster. Note that because the manual grouping method and the NLP clustering segmented semantic space differently, two different clusters could be linked to the same manual grouping if a plurality of responses in both clusters were grouped together. A given response was manually coded into the same manual group as the plurality of its cluster `r round(perAgreement,3)` of the time. Treating the manual grouping method and the clustering method as two different "raters" we found a Cohen's kappa coefficient of `r round(kappaAgreement$value,3)` suggesting strong agreement between the clustering and the manual grouping methods.


```{r decisionpredict, echo=F,warning=F,message=F}
# cluster_df 

# calcualte average value of possibility generation responses for each cluster
pg_clusters <- cluster_df %>%
...
decision_clusters <- cluster_df %>% 
  filter(source == 'decision' & clustering_method == 'numElbow') %>% 
  group_by(context, cluster, cluster_name) %>% 
...
# Model: Sample k samples from pg for a given context.
# assign clusters, select option with highest cluster value. Summarise to get proportion of being chosen for each cluster
...
run_decision_model <- function(k, df, pg_clusters, sample_sets) {
  ...
  decision_model <- df %>% 
    ...
    # takes the response with the highest avg value of cluster
...
# Use purrr::map to run the function for each k value
decision_results <- map_df(k_values, ~ run_decision_model(.x, cluster_df, pg_clusters, sample_sets))
...
```{r decisiongraph}
# run decision model using k = 2 as determined by above analysis

# selects results from decision model with k=2
cor_result <- with(decision_results %>% filter(k==2), cor.test(model_proportion, proportion_decision))
cor_result
```
Having verified that the clustering method generated similar results as human raters when using the same number of clusters, we next sought to select the number of clusters using a more data-driven method. For each context, we generated an 'elbow plot' to determine the appropriate number of clusters. 

```{r decisionplot, echo=F, warning=F, message=F, fig.width=6.5,fig.height=5.25}
figDecision <- decision_results %>% filter(k == 2) %>% 
...
# ggsave(figDecision,filename = "../figs/figDecision.png", width = 10, height = 8,units = "in")
...
# ggsave(c13.h,file="figs/fig3.jpg", width=8, height=6, units="in")
...
# ggsave(fig4,file="figs/fig4.jpg",width = 10,height = 8)
...
fig5 <- causalSum %>% mutate(context=factor(context)) %>%
...
# ggsave(fig5,file="figs/fig5.jpg", width = 10, height = 8)
...
# ggsave(figS3,file="figs/figS3.jpg", width = 10, height = 8)
...
# ggsave(figS4, file="figs/figS4.jpg",width=7, height=4, units="in")
